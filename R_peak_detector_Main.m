...
%Read root
fs=200;
%folder_root='C:\Pace_project\PACE_ReviewedData_Combined\'
folder_root='C:\Pace_project\Data_Copy_For_HRV_Analysis\PACE_ReviewedData_Combined\'
%excel_root='C:\Pace_project\PACE_ReviewedData_Combined\Excel_results\'
excel_root='C:\Pace_project\Data_Copy_For_HRV_Analysis\PACE_ReviewedData_Combined\Excel_results\';
...
parfor folder_dat=1:anz
	...
	excelName=strcat(excel_root,tachofolder(folder_dat).name,'.xls');
        ...
        fprintf(fid1,' Number over 15 Percent \t');
        fprintf(fid1,' Number over 12 Percent \t');
	...
	% load the file and check if the data exist
		...
		if isfield(matfile,'bw_data')
			...
			%No_segments=floor(length(matfile.bw_data)/36000);
            ...
            % read each 3 minutes segment ( this is the part you need to change if you want to make a sliding search of the 3 minutes
            algo_num = 1;
            interval_t = 36000;
            good_percent = 0.15; % change this to 0.15 if necessary
            check_percent = 0.15;
            check_percent2 = 0.12;
            check_percent3 = 0.10;
            check_percent4 = 0.08;
            ...
            while true 
              ...
              while pos_start+interval_t < sequence_length
				...
				% Check if there is something larger than the 20% difference of the mean (This is the part you want to change if you want to use a different method, i.e., checking each a previous RR interval instead of the global median)
                use_median = false;
			...
			if isempty(pos_d)
              %% Original line:
              %  good_segment=seg_dat;
              good_segment=start_index; % not sure this applies with sliding window approach
              good_data=data;
              good_Rpeaks=peakloc;
              ...
              %% Original line:
              %  time_3min=(seg_dat*3)-2; 
              time_3min=(start_index-1.0)/interval_t; % not sure this is correct
...
% The other methods to find the QRS complex are below
...
function peakloc=rpeak_pan_tompkins(data,fs)
    %Rpeak detection using Pan-tomkins method (This is the part you want to replace for another method)
    % Also the threshold of 0.01 can be modified for each patient.
...
% Pan_tompkin method 2
%[qrs_amp_raw,qrs_i_raw,delay]=pan_tompkin2(data,fs);
...
%% Rpeak detection based on parabolic fitting
Rx=rpeak(data,fs);
...
% Rcorrection based on R-peak neighborhood maximum to minimum
    ...
    Rx_temp=Rx(Rx_loc);
    Rx=Rx_temp;
     Rx = PeakCorrection(data,Rx,'absPeak');
...
Median1=median(diff(peakloc))
Media2=median(diff(A.RR_reviewed))
Media3=median(diff(qrs_i_raw))
Median4=median(diff(Rx))
...
plot(data)
hold on
plot(qrs_i_raw,data(qrs_i_raw),'ro')
hold off
%}
